...
| 04 | **SUPEX를 현실로** | 모두가 같은 방송 무대에 오르려면 무엇을 해야 하는가? | 스케치북을 열고 문제부터 결과까지 스크롤 | 함께 완성한 무대와 피날레 | 약 70초 |
...
촬영 직전, 휠체어 이용 출연자가 20cm 높이의 방송 무대 앞에서 멈춥니다. 출연자와 제작진이 경사로의 기준을 정하고 함께 조립하고 직접 시험하고 설치를 끝내 같은 무대에서 방송을 시작합니다.

```text
SUPEX 스케치북 → 무대 앞의 문제 → 탁월 → 단합 → 적용 → 실행 → 같은 무대
```

- 첫 화면에서 작은 검정 스케치북이 열리고 실제 문제 장면 안으로 들어갑니다.
- 밝은 풀스크린 사진 위 검정 캡션으로 문제와 네 행동을 한 장면씩 보여줍니다.
- 탁월은 경사와 동선 측정, 단합은 공동 조립, 적용은 실제 시험과 수정, 실행은 최종 고정과 통과를 보여줍니다.
- 데스크톱과 모바일에 맞춘 별도 비주얼을 사용합니다.
- 마지막에는 출연자와 MC가 같은 무대에서 방송을 시작하고 `SUPEX`가 채워집니다.
...
4. **현실화** — SUPEX를 현실로에서 스케치북을 열고 무대 문제를 해결하는 네 행동과 피날레를 보여줍니다.
...
- SUPEX를 현실로는 종이 넘김음, 네 단계 전환음, 피날레 화음으로 현실화 과정을 연결합니다.
...
└── sk-supex-3/              # Project 4 · 같은 방송 무대를 만드는 스크롤 스토리
```
