...
탁월·단합·적응·실행을 개인의 선택, 현재의 업무 행동, 직접 수행하는 게임, 하나의 아이디어가 완성되는 과정으로 연결했습니다. 네 제품은 각기 다른 질문으로 SUPEX를 경험합니다.
...
| **적응** | 새로운 가능성을 실제 업무에 맞게 시험하고 다듬는 힘 | 작은 범위에서 확인하고 결과를 반영한다 |
...
최근 업무에서 실제로 선택하는 행동을 네 문항으로 확인합니다. 탁월·단합·적응·실행은 각각 40·60·80·100점 가운데 하나로 계산되며, 네 점수와 개인 정보가 인터랙티브 라이선스에 담깁니다.
...
- 사진을 업로드하면 정장과 SK 라펠핀이 반영된 포트레이트를 생성합니다.
...
탁월·단합·적응·실행을 서로 다른 네 가지 행동 게임으로 바꾼 약 1분 분량의 시네마틱 아케이드입니다.

| 순서 | 게임 | 플레이 | 경험하는 SUPEX |
| --- | --- | --- | --- |
| 탁월 | 기준 맞추기 | 바깥 원부터 네 개의 회전 표시를 기준선에 맞춘다 | 정한 기준에 정확히 도달한다 |
| 단합 | 하나로 잇기 | 0에서 시작해 손을 떼지 않고 1부터 10까지 교차 연결한다 | 각자의 일을 하나의 흐름으로 잇는다 |
| 적응 | 변화 반영하기 | 3×3 보기에서 가운데 목표와 같은 단어를 빠르게 고른다 | 바뀐 기준을 확인하고 즉시 반영한다 |
| 실행 | 순서대로 끝내기 | 흩어진 숫자를 1부터 12까지 완성한다 | 정한 일을 바로 시작해 끝까지 마친다 |

```text
인트로 → 탁월 → 단합 → 적응 → 실행 → 종합 점수와 코멘트 → 다시 시작
```

- 실제 플레이 제한시간은 총 46초입니다.
