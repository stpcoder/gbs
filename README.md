...
SUPEX를 **발견하고, 측정하고, 플레이하는** 세 가지 방송용 인터랙티브 경험입니다.

탁월·단합·적용·실행을 개인의 선택, 현재의 업무 행동, 직접 수행하는 게임으로 연결했습니다. 세 제품은 각기 다른 질문으로 SUPEX를 완성합니다.
...
```text
01 발견                         02 진단                         03 체험
나의 방식 찾기      →          현재 행동 측정      →          네 가지 힘 플레이
12가지 TYPE                    4가지 SCORE                     4가지 GAME
```
...
세 제품을 이어서 시연하면 SUPEX가 개인의 성향에서 현재 행동으로, 현재 행동에서 실제 수행으로 확장됩니다.
...
세 제품은 D장조 펜타토닉의 맑은 유리·피아노 음색을 공유합니다.
...
이 저장소는 세 제품을 연결하는 허브입니다. 각 제품은 독립 저장소와 Vercel 프로젝트를 유지하며 Git 서브모듈로 연결됩니다.
...
이미 복제한 저장소에서는 다음 명령으로 세 프로젝트를 받습니다.
...
다른 두 제품도 같은 방식으로 실행하며 기본 포트가 사용 중이면 Next.js가 다음 포트를 자동으로 선택합니다.
...
런타임 AI는 SUPEX LICENSE의 짧은 해석과 포트레이트 생성에 사용합니다. 나의 SUPEX와 SUPEX RUN은 미리 제작한 비주얼 자산과 로컬 결정 로직으로 동작합니다.
