...
| --- | --- | --- | --- | --- | --- |
| 01 | **나의 SUPEX** | 나는 어떤 방식으로 최고의 결과를 만드는가? | 네 가지 업무 상황에서 행동 선택 | 12가지 SUPEX TYPE | 약 85초 |
...
| 04 | **SUPEX REAL** | 한 줄의 생각은 어떻게 모두의 하루가 되는가? | 스케치와 현실을 비교하고 네 단계 스크롤 | 사람들이 실제로 머무는 공간과 피날레 | 약 60초 |
...
| SUPEX REAL | [sk-supex-3.vercel.app](https://sk-supex-3.vercel.app) | [stpcoder/sk-supex-3](https://github.com/stpcoder/sk-supex-3) |
...
네 번의 선택으로 자신이 결과를 만드는 방식을 찾습니다. 각 선택은 탁월·단합·적용·실행 점수에 반영되고, 가장 강한 두 힘의 순서가 12가지 타입 가운데 하나를 결정합니다.

```text
인트로 → 4개 상황 선택 → 선택 장면 결합 → SUPEX 완성 → 개인 TYPE → 다른 성향 보기
```

- 질문마다 세 개의 현실적인 선택지를 제공합니다.
- 선택지에 집중하면 배경 장면이 즉시 바뀝니다.
- 선택 후 붉은 천 매치컷으로 다음 상황이 이어집니다.
- 마지막에는 네 장면이 모이며 `SUPEX` 글자가 채워집니다.
...
![SUPEX REAL 대표 비주얼](./docs/images/project-4-hero.jpg)

빈 도심 마당에 그린 작은 그늘과 벤치가 실제로 사람들이 머무는 장소가 되기까지의 과정을 포인터와 스크롤로 경험합니다. 같은 사람, 같은 공간, 같은 붉은 선이 처음부터 끝까지 이어집니다.

```text
스케치 ↔ 현실 → 탁월 → 단합 → 적용 → 실행 → 모두의 하루
```

- 첫 화면에서 포인터 또는 모바일 슬라이더로 스케치와 현실을 비교합니다.
- 스크롤 속도에 맞춰 같은 공간의 네 행동 장면이 전환됩니다.
- 탁월은 사람을 위한 완성 기준, 단합은 하나로 모인 그림, 적용은 현장에서의 수정, 실행은 사용할 수 있는 결과를 보여줍니다.
- 데스크톱과 모바일에 맞춘 별도 비주얼을 사용합니다.
- 마지막에는 사람들이 실제로 머무는 공간과 함께 `SUPEX`가 채워집니다.

이 경험은 **생각을 실제 결과로 만드는 SUPEX의 과정**을 보여줍니다.

![SUPEX REAL 피날레 비주얼](./docs/images/project-4-finale.jpg)

[제품 상세 보기](./docs/PRODUCTS.md#04--supex-real) · [지금 체험하기](https://sk-supex-3.vercel.app)
...
4. **현실화** — SUPEX REAL에서 스케치와 현실을 비교하고 네 행동과 피날레를 보여줍니다.
...
- SUPEX REAL은 스케치 비교음, 네 단계 전환음, 피날레 화음으로 현실화 과정을 연결합니다.
...
└── sk-supex-3/              # Project 4 · 스케치에서 실제 결과까지
```
...
런타임 AI는 SUPEX LICENSE의 짧은 해석과 포트레이트 생성에 사용합니다. 나의 SUPEX와 SUPEX RUN, SUPEX REAL은 미리 제작한 비주얼 자산과 로컬 인터랙션으로 동작합니다.
