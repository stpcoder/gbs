...
| 04 | **SUPEX REAL** | 하나의 아이디어는 어떻게 실제 결과가 되는가? | 스케치를 직접 완성하고 제작 과정 스크롤 | 아이디어가 실제 제품이 되는 시네마틱 피날레 | 약 60초 |
...
하나의 러닝화가 스케치에서 실제 움직임이 되기까지의 과정을 마우스와 스크롤로 완성합니다. 같은 제품이 형태를 유지한 채 폼 목업, 재질 렌더, 팀 리뷰, 현장 테스트, 실제 사용으로 이어집니다.

```text
스케치 완성 → 탁월 → 단합 → 적용 → 실행 → SUPEX 피날레
```

- 첫 화면에서 스케치를 직접 지우며 실제 제품을 드러냅니다.
- 스크롤 속도에 맞춰 이미지가 연속 장면처럼 전환됩니다.
- 탁월은 완성의 기준, 단합은 하나로 맞춘 목표, 적용은 테스트 결과의 반영, 실행은 끝까지 이어가는 행동을 보여줍니다.
- 데스크톱과 모바일에 맞춘 별도 비주얼을 사용합니다.
- 마지막에는 완성된 제품과 함께 `SUPEX`가 채워집니다.
...
4. **현실화** — SUPEX REAL에서 스케치를 완성하고 제작 과정과 피날레를 보여줍니다.
...
- SUPEX REAL은 스케치 완성음, 네 단계 전환음, 피날레 화음으로 제작 과정을 연결합니다.
