...
| SUPEX를 현실로 | 문제를 실제 결과로 해결하는 과정 | 스케치북과 스크롤 | 모두가 오르는 방송 무대와 피날레 | 네 행동이 한 결과로 이어지는 흐름 체험 |
...
촬영 직전 발견한 20cm 무대 높이를 출연자와 제작진이 함께 해결합니다. 같은 방송실, 같은 출연자, 같은 경사로가 문제부터 최종 방송까지 이어집니다.

### 전체 흐름

1. 작은 검정 SUPEX 스케치북을 스크롤로 엽니다.
2. 스케치와 실제 무대 문제를 한 권 안에서 확인하고 사진 속으로 들어갑니다.
3. 문제, 탁월, 단합, 적용, 실행의 장면을 차례로 확인합니다.
4. 출연자와 MC가 같은 무대에서 방송을 시작하는 장면과 `SUPEX` 피날레를 확인합니다.

### 네 단계

| SUPEX | 장면 | 화면에서 확인하는 행동 |
| --- | --- | --- |
| 탁월 | 경사·회전 폭·카메라 동선 측정 | 스스로 오를 수 있는 완성 기준을 정한다 |
| 단합 | 출연자와 제작진의 공동 조립 | 서로 다른 경험과 역할을 한 결과에 모은다 |
| 적용 | 임시 경사로의 실제 통과 시험 | 사용 중 발견한 문제를 현장에서 고친다 |
| 실행 | 마지막 연결부 고정과 최종 통과 | 방송 전에 설치와 안전 점검을 끝낸다 |

### 인터랙션과 비주얼

- 3D 표지와 세 장의 종이가 열리는 240svh 스케치북 포털
...
모든 장면은 Google Vertex AI의 `gemini-3-pro-image`로 생성한 뒤 WebP로 정리했습니다. 방송실, 휠체어 이용 출연자, 제작진, 목재 경사로를 모든 장면의 공통 요소로 사용합니다. 모델·프롬프트·결과 파일의 해시를 생성 매니페스트에 기록합니다.
