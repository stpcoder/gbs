...
| SUPEX REAL | 결과가 만들어지는 과정 | 마우스와 스크롤 | 제품 완성과 피날레 | 아이디어가 현실이 되는 흐름 체험 |
...
하나의 아이디어가 탁월·단합·적용·실행을 거쳐 실제 결과가 되는 과정을 직접 완성합니다. 사용자의 조작으로 제품이 만들어지는 흐름을 이어갑니다.

### 전체 흐름

1. 마우스 또는 손가락으로 스케치를 지워 실제 러닝화를 드러냅니다.
2. 스크롤하며 스케치, 폼 목업, 재질 렌더를 연결합니다.
3. 탁월, 단합, 적용, 실행의 현장 장면을 차례로 확인합니다.
4. 완성된 제품과 `SUPEX` 피날레를 봅니다.

### 네 단계

| SUPEX | 장면 | 화면에서 확인하는 행동 |
| --- | --- | --- |
| 탁월 | 제품 기준 검토 | 무게·접지력·내구성의 목표를 분명하게 정한다 |
| 단합 | 부품 결합 | 디자인·기술·사용자 관점을 한 제품에 모은다 |
| 적용 | 젖은 트랙 테스트 | 현장에서 찾은 개선점을 다음 설계에 반영한다 |
| 실행 | 실제 사용 | 담당과 일정을 정하고 첫 제품을 현장에 선보인다 |

### 인터랙션과 비주얼

- 캔버스 지우기 방식의 스케치 공개
- 620svh 고정 스크롤 구간과 장면별 크로스페이드
- 같은 러닝화가 형태를 유지하는 연속 서사
- 16:9 데스크톱과 9:16 모바일 전용 이미지
- 모션 감소 설정 지원
- 스케치 완성, 단계 전환, 피날레에 맞춘 Web Audio 차임

### 이미지 제작

모든 장면은 Google Vertex AI의 `gemini-3-pro-image`로 생성한 뒤 WebP로 정리했습니다. 밝은 디자인 스튜디오, 한국인 팀, 웜 화이트 러닝화와 붉은 밑창을 공통 요소로 사용합니다.
